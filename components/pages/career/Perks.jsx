...
export default function Perks() {
  ...
  console.log("Preks", Preks);

  useEffect(() => {
    GetOurPreks()
  }, [])

  const GetOurPreks = async () => {
    var resp = await getCmsContent("joinusteammiddle");
    console.log("🚀 ~ GetOurPreks ~ resp:", resp)
    if (resp?.status) setPreks(resp.data);
  };
  return (
              ...
              >
                ...
                <g clipPath="url(#clipping)">
                  <image
                    ...
                    // src={`${config.IMG_URL}/cmsimg/${Preks?.img}`}
                    width="200"
                    height="200"
                ...
                className="mt-8 inline-block w-72 rotate-[8deg] sm:w-full lg:w-[24rem] xl:w-[35rem] object-fit:contain rounded-[100px]"
                alt="team"
              />
              <Image
                width={740}
