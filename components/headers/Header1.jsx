...
export default function Header1() {
  ...
  const getInitialSeviceFee = async () => {
    var fees = await Contract.getServiceFees();
    console.log("getServiceFees",fees,{
      ServiceFees: {
          buyerFees :  Number(fees.buyerFee) / 100,
          sellerFees :  Number(fees.sellerFee) /100 ,
          owneraddress : fees?.owner ? fees?.owner?.toString() : ""
      },
  });
    if (fees) {
        dispatch({
  ...
  const initialConnectWallet = async (type, title) => {
    console.log("initialConnectWallet", type);
    var accountDetails = await connectWallet(
      type ?? localStorage.getItem("walletConnectType")
    );
              ...
              <div className="contentTop chatset">
                ...
                Searchdata?.collection?.msg?.length > 0 ? (
                  ...
                  <>
                    ...
                    {Array.isArray(Searchdata.user?.msg) &&
                      Searchdata.user?.msg?.length > 0 && (
                        <div className="content">
                          ...
                          {Searchdata.user?.msg?.map((value) => (
                            <>
                              <div
                                className="searchset hover:text-accent justify-content-start pt-4 searchCursor cursor-pointer"
                                onClick={() => {
                                  Setval("");
                    ...
                    {Searchdata.token?.data?.length > 0 && (
                      <div className="content">
                        ...
                        {Searchdata.token?.data.map((value) => (
                          <>
                            <div
                              className="searchset pt-4 hover:text-accent searchCursor cursor-pointer"
                              onClick={() => {
                                Setval("");
                                SetSearch();
                                push(
                  ...
                  <span className="error_msg emsg pTag text-center">
                    No data Found
                  </span>
                )}
              </div>
                          ...
                          Searchdata.user?.msg?.length > 0 && (
                            <div className="content">
                              ...
                              {Searchdata.user?.msg?.map((value) => (
                                <>
                                  <div
                                    className="searchset hover:text-accent pt-4 searchCursor"
                                    onClick={() => {
                                      Setval("");
                        ...
                        {Searchdata?.token?.data?.length > 0 && (
                          <div className="content">
                            ...
                            {Searchdata?.token?.data?.map((value) => (
                              <>
                                <div
                                  className="searchset pt-4 hover:text-accent searchCursor"
                                  onClick={() => {
                                    Setval("");
                                    SetSearch();
                                    push(
